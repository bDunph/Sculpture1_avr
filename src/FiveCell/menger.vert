...
void main() {
	...
	//gl_Position = MVEPMat * vec4(position, 1.0);
...
//*********************************************************************************************
	...
	vec4 eyeInv = vec4(-viewEye[3].xyz, 1.0);
	eye = invView * invEye * eyeInv;

	//vec2 screenPos = vec2((position.x - eyeMat[3].x) * fovYScale * aspect, position.y * fovYScale);
	vec2 screenPos = vec2((position.x - viewEye[3].x) * fovYScale * aspect, position.y * fovYScale);
